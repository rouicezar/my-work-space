...
Scope verified: the widened P3-T16 is fully implemented and machine-verified. (A) `forma_ai/processes.py: herdr_process_spec` builds the headless-server launch contract `("--session", session_name, "server")` (matches the project's own live Herdr integration-test harness which launches `[binary, "--session", name, "server"]`); (B) `forma_ai/runtime.py` `RuntimeRecord`/`RuntimeManager` gained a 4th "herdr" role — spawned first, stopped first, `status()` reports `herdr_alive`, and the rollback path terminates a failed Herdr start; (C) `scripts/supervisor.py` added `_installed_herdr_executable` (reuses `forma_ai/artifacts.py` load_component/select_artifact/verify_file against `config/upstreams.json`, no new activation record), `_herdr_probe`, wired Herdr into `start-runtime`, and switched `herdr-snapshot` from `--socket-path` to `--root` with a fail-closed pre-check on `herdr_alive` (returns `HERDR_NOT_RUNNING`/stale without connecting); (D) `SupervisorProtocol.swift` added `herdrAlive` to `RuntimeStatusPayload`, `workspaceID`/`tabID` to `RuntimePresentedAgent`, `herdrSnapshot(rootURL:)` and `startRuntime(osMajor:architecture:)`, and `RuntimePresentationProvider`; (E) `FormaAIApp.swift` removed `AgentTaskViewState.parallelPreview` and the "Preview fixture · not runtime evidence" label, and `ManifestOverview` now loads real Herdr snapshot data through `RuntimePresentationProvider.herdrSnapshot(rootURL:)` with fail-closed disconnect/empty states and only schema-present fields (pane/workspace/tab/terminal/state/revision).
Verification: focused pytest `tests/test_processes.py tests/test_runtime.py tests/test_supervisor.py` → 65 passed + 7 subtests; full `python3 -m unittest discover tests` → 311 tests OK (1 expected opt-in Semantica skip); full `swift test --package-path prototypes/packaging` → 44 tests passed (2 real-Keychain environment-gated skips); `git diff --check` clean; no `forma-*` named-session residue. The runtime-card acceptance (runtime mode contains no fixture fallback; user sees authoritative Herdr agents) and the fail-closed snapshot acceptance (`herdr-snapshot` does not connect when Herdr is not running) are both met by tested code. Note: the previous "next action" to run a disposable named-session probe is now unnecessary — the `--session <name> server` launch form is already proven by the repo's own live Herdr integration tests.
Upstream decision: Herdr remains sole authority for terminal/pane/agent identity and state; the product adds only start/stop/digest-verify of the official binary and a fail-closed snapshot query. No competing state machine or fabricated display fields. `forma_ai/artifacts.py` reuse avoids a new install-layout/activation record (Herdr manifest `install_mode` is `verified_release_binary`).
Files changed: `forma_ai/processes.py`, `forma_ai/runtime.py`, `scripts/supervisor.py`, `prototypes/packaging/Sources/FormaAIApp/FormaAIApp.swift`, `prototypes/packaging/Sources/SupervisorProtocol/SupervisorProtocol.swift`, `prototypes/packaging/Tests/SupervisorProtocolTests/SupervisorProtocolTests.swift`, `tests/test_processes.py`, `tests/test_runtime.py`, `tests/test_supervisor.py`, `docs/plans/2026-08-31-multi-agent-workbench-master-plan.md`, `docs/TASK_HANDOFF.md`, and the mandatory Chinese mirrors.
Commit and push: local commit created; NOT pushed (user has not requested a push).
Blocked items: none for P3-T16. The manual GUI window walkthrough remains a P3-T17 manual-review item, not a P3-T16 blocker.
Next exact action: claim P3-T17 (close the real Herdr multi-agent phase — full real integration suite and manual task review), then the P3 milestone can be marked verified.
Secret/external-write status: no credentials, cloud/model calls, production/default sessions, or external network; only disposable local named Herdr sessions (self-cleaned) and repo-local test resources. The pinned Herdr binary was only queried read-only (`--help`/`server --help`/`session --help`); no server or default session was started.

## 2026-09-03 - P3-T16 - pi takeover (new executor)

Executor: pi agent (new executor; continuing the existing P3-T16 claim previously held by Qoder)
Starting git state: `main` is 1 commit ahead of `origin/main` at `35ac97e` (the synchronized P3-T16 claim commit, UNPUSHED). 9 working-tree files carry the P3-T16 implementation: `forma_ai/processes.py`, `forma_ai/runtime.py`, `scripts/supervisor.py`, `prototypes/packaging/Sources/FormaAIApp/FormaAIApp.swift`, `prototypes/packaging/Sources/SupervisorProtocol/SupervisorProtocol.swift`, `prototypes/packaging/Tests/SupervisorProtocolTests/SupervisorProtocolTests.swift`, `tests/test_processes.py`, `tests/test_runtime.py`, `tests/test_supervisor.py`. This is the preserved prior P3-T16 implementation; no user-new-committed state exists to discard.
Trigger: user instructed a full project takeover, first requiring the takeover/control docs to be read, the dev-environment package dependencies to be confirmed and installed, and all prep completed before formally assuming the current task. User approved the package set to install: `pytest` (pip dev dependency), `@narumitw/pi-plan-mode` (pi official), `pi-simplify` (pi official).
Scope (unchanged, inherited from the widened P3-T16): replace the preview Agent card fixture and the always-shown "Preview fixture · not runtime evidence" label with real Herdr snapshot data in runtime mode; add Herdr process lifecycle management (start/stop/status), since `RuntimeManager` previously managed only omlx/broker/memory and the `herdr-snapshot` CLI required a caller-supplied `--socket-path` with no executable resolution/verification.
Pre-takeover verification (new facts from the actual repo and the pinned Herdr binary):

- Python P3-T16 target tests pass under pytest: `tests/test_processes.py tests/test_runtime.py tests/test_supervisor.py` → 65 passed + 7 subtests. Full `python3 -m unittest discover tests` is also green (311 tests, 1 expected skip).
- Toolchain: Python 3.12.6 (pyenv), Swift 6.3.3 (SPM `SupervisorProtocol` target builds clean), PyYAML 6.0.3 present, pytest 9.1.1 just installed.
- Herdr CLI argument form (verified against the cached pinned binary at `~/Library/Application Support/Forma AI/cache/downloads/herdr-macos-aarch64` via `--help`/`server --help`/`session --help`): `herdr server` (bare) is documented as "Run as headless server"; `herdr server stop|reload-config|agent-manifests|update-agent-manifests|reload-agent-manifests` control a running server; `--session <name>` = "Use or create a named persistent session"; the session socket resolves to `~/.config/herdr/sessions/<name>/herdr.sock`. The current uncommitted `herdr_process_spec` builds `("--session", session_name, "server")` → `herdr --session forma-workbench server`; this ordering still requires a one-time disposable named-session probe to confirm it launches the headless server in that named session (precisely the verification the prior handoff flagged as the next action).
Upstream search result: re-verified — `forma_ai/artifacts.py` (`load_component`/`select_artifact`/`verify_file`) is reused as-is to digest-verify the cached Herdr binary against `config/upstreams.json`, matching oMLX's `_installed_omlx_executable` pattern; no new install-layout/activation record is created (Herdr manifest `install_mode` is already `verified_release_binary`).
License obligation: Apache-2.0 external binary/socket reuse only; no upstream source, assets, names, or trademarks copied.
Integration decision: Herdr remains sole authority for agent/pane/terminal state; product adds only start/stop/digest-verify of the official binary and a fail-closed snapshot query; no competing state machine is introduced.
Planned verification (inherited): run the one-time disposable named-session probe to confirm the server-start form; then `pytest tests/test_runtime.py tests/test_supervisor.py tests/test_herdr_adapter.py`; `swift test` for `SupervisorProtocolTests`; manual runtime-mode UI check (no "Preview fixture" label; Herdr-not-running shown honestly); bilingual parity; `git diff --check`.
Commit and push: no push without an explicit user request; the existing P3-T16 working-tree work is preserved in place.
Next exact action: run the disposable named-session probe to confirm the correct headless-server launch form for `herdr_process_spec`; correct the spec to the verified form, finish any remaining Swift wiring, run full verification, and write the P3-T16 closeout in both control languages.
Secret/external-write status: no credentials, cloud/model calls, production/default sessions, or external network; only disposable local named Herdr sessions (self-cleaned) and repo-local test resources.
