...
This is a pinned upstream capability map, not runtime acceptance. The official artifact was downloaded, digest-verified, and executed for version/protocol/schema checks on 2026-09-01 (P3-T10); the official socket transport has since run real `ping` and `session.snapshot` requests in isolated test sessions with a fail-closed protocol gate (2026-09-01, P3-T11); and two real fixture agents have now run in parallel through `workspace.create` + `pane.split` + `pane.report_agent` on distinct panes and working directories, with verified parallel execution, pane-exact graceful cancel, and no input/state/artifact/cross-pane output leakage (2026-09-01, P3-T12). No event subscription, presentation binding, real wait/blocked/artifact surface, or detach/reconnect/recovery flow has run yet (P3-T13+). Every capability below is sourced from the official v0.8.2 release, pinned source, bundled schema, or versioned documentation.
...
- CLOSED 2026-09-01 (P3-T12): two real fixture agents ran in parallel through the official runtime in one isolated named session — `workspace.create` → root pane, `pane.split` → second pane with distinct cwd, `pane.send_text` + `pane.report_agent` per agent, states observed via `agent.get` and `session.snapshot` (`working` while running, both `idle` at close). Parallelism proven by timing (agent B finished inside agent A's sleep window), graceful cancel proven pane-exact (agent A's `ctrl+c` interrupted its sleep; the post-sleep leak steps never executed), and isolation proven across input, artifacts, and pane text. Evidence: `evidence/upstream/herdr-v0.8.2-two-fixture-agent-isolation-verification-2026-09-01.md`.
- REMAINING: no event subscription, presentation binding, real wait/blocked/artifact-read surface, or detach/reconnect/recovery flow has run in Forma AI yet; that is P3-T13+ scope. Cancellation is proven for graceful interrupt on reported fixture agents only; forced termination and confirmation policy levels remain open.
