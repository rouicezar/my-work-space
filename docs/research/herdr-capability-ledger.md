...
- REMAINING: event subscription primitives exist, but no forced socket-loss → stale/unknown presentation → fresh snapshot reconciliation → resubscribe proof or runtime UI binding has run yet; that is P3-T13 scope. Real wait/blocked/artifact-read, forced termination, provider-native session resume, detach/reconnect, and confirmation policy levels remain P3-T14+.
