...
| O3 | Herdr availability check | `forma_ai/herdr_adapter.py` `availability()` uses the injected official transport probe when configured and keeps binary-only discovery explicitly `unknown` otherwise | Herdr `ping` over the official socket transport | Apache-2.0 | Thin adapter; P3-T11 reduction completed | — |
| O4 | Herdr event subscription, wait/read/cancel, and presentation reconnect | `forma_ai/herdr_transport.py` and `forma_ai/herdr_presentation.py`; direct socket envelopes plus snapshot-first stale/unknown UI reconciliation | Herdr `events.subscribe`/`events.wait`, `session.snapshot`, `agent.wait/read`, `pane.process_info/send_keys/close` | Apache-2.0 | Thin adapter plus justified product-owned approval/presentation policy; detach/native resume remains upstream-backed P3-T15 work | P3-T15 |
| O4A | Historical client-reported fixture launcher | `forma_ai/herdr_adapter.py` `spawn_reported_task`; only two unit tests still call it after corrected P3-T12 moved to official `agent.start` | Herdr `agent.start` is the selected native launch entry point; `pane.report_agent` remains an upstream reporting hook, not the product task launcher | Apache-2.0 | Removal candidate: obsolete production-surface test seam duplicates the selected launch path and should move out of `HerdrAdapter` before P3-T15 | P1-T09 correction gate |
...
5. **F5 (P1-T09 correction before P3-T15) — obsolete reported-task launcher**: `spawn_reported_task()` is no longer used by product or live integration paths after the P3-T12 correction. Its `pane.send_text → pane.report_agent → agent.get` wrapper duplicates the selected `agent.start` task-launch path and survives only for two unit tests. Remove it from the production adapter or provide new non-test evidence for retaining it; do not carry both launch authorities into P3-T15.

## Verdict

Most audited product-owned runtime-like capabilities remain justified, thin upstream adapters, or scheduled reductions. The 2026-09-02 follow-up found one concrete removal candidate: the obsolete `spawn_reported_task()` production method retained from the superseded P3-T12 client-reporting fixture. Upstream-first work must resolve F5 before P3-T15; this does not change the product goal or architecture.
