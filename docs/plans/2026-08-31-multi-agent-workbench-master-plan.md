...
Current task: none (P3-T13 verified; executing-plans review checkpoint)
Current status: P3-T13 verified — real server-side socket loss marks presentation `stale` and old agents `unknown`; recovery reads a fresh authoritative snapshot before resubscribing, removes vanished pane identities, and resumes live state on the new pane. Because live v0.8.2 status events omit revision, they trigger a snapshot reconciliation instead of receiving an invented product sequence. Supervisor and Swift runtime-provider mappings are verified with no preview fallback.
Next action: after this checkpoint, claim P3-T14 and prove real wait, blocked, bounded artifact-read, graceful cancel, and force-cancel escalation using official Herdr surfaces. Local Qwen integration follows the P3-T14～T16 runtime closure; DeepSeek remains approval-gated P5 work.
