...
Current task: P3-T11 (verified; P3-T12 is next)
Current status: verified — the thin Herdr adapter is bound to the official Herdr v0.8.2 socket transport (`forma_ai/herdr_transport.py`): one connection per request, response location by `result`/`error` keys, fail-closed protocol-20 gate via `ping`, socket resolution explicit path → `HERDR_SOCKET_PATH` → `HERDR_SESSION` → default; adapter `availability()` now reports real ping evidence (F2 availability half closed; binary-presence behavior preserved without a probe). Live `ping` (pong version 0.8.2, protocol 20) and `session.snapshot` proven against the verified binary in uniquely named test sessions (created, stopped, deleted by the tests; default session never started). Evidence: `evidence/upstream/herdr-v0.8.2-socket-transport-verification-2026-09-01.md`; implementation/evidence commit `f9191bf`; 271 tests OK, 1 skipped
Next action: claim P3-T12 — two real isolated fixture agents through Herdr using only schema-documented methods (`pane.split` + `agent.start`/`pane.send_text`, never `pane.run`; ledger also records the `pane.graphics.stream` runtime/schema discrepancy) with distinct panes/runs and no input, state, artifact, or cancel leakage; P4-T14 remains the default next frontend task and unchanged
