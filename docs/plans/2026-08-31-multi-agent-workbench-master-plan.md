...
Current task: P3-T13
Current status: in_progress — Codex resumed Qoder's committed snapshot/subscription/wait slice after the P3-T12 correction. The user explicitly stopped shell polishing and approved the real-closure order. Acceptance is forced socket loss → immediate stale/unknown presentation → fresh authoritative snapshot reconciliation → resubscribe → resumed live transitions, followed by Supervisor and Swift runtime-provider mapping.
Next action: add the failing forced-disconnect/reconciliation contract, implement fail-closed reconnect ordering and revision/sequence rejection, then bind the verified envelope through SupervisorProtocol and the Swift `RuntimePresentationProvider`. Local Qwen through the already verified oMLX route remains the preferred real-model path; DeepSeek remains approval-gated P5 work.
...
| P3-T13 | in_progress | Bind snapshot and event subscriptions to the workbench presentation provider | adapter, Swift protocol, UI | Forced socket loss marks presentation stale/unknown; fresh snapshot reconciles before resubscribe; live transitions then resume | UI cannot override Herdr semantic state |
