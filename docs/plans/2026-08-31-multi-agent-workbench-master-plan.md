...
Current task: P3-T11
Current status: in_progress (claimed by Qoder agent 2026-09-01; takeover entry in `docs/TASK_HANDOFF.md`)
Next action: implement `forma_ai/herdr_transport.py` (newline-delimited JSON over the Unix-domain socket; protocol 20 with fail-closed compatibility gate; response location by `result`/`error` keys because error responses can carry an empty `id` and event lines carry none; socket resolution explicit path → `HERDR_SOCKET_PATH` → `HERDR_SESSION` → default `~/.config/herdr/herdr.sock`, named sessions at `~/.config/herdr/sessions/<name>/herdr.sock`) plus the adapter probe seam so `availability()` reflects a real ping (F2 availability half); live `ping` and `session.snapshot` tests against the verified binary in a uniquely named test session (started, stopped, and deleted by the tests); wire contract empirically confirmed against the official binary; P4-T14 remains the default next frontend task and unchanged
...
| P3-T11 | in_progress | Bind the thin adapter to the official Herdr socket transport | Herdr adapter and transport tests | Real `ping`, `session.snapshot`, and compatible protocol response | Runtime truth comes from Herdr |
