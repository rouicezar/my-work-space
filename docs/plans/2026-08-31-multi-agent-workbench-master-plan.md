...
Current task: P3-T13 (in_progress)
Current status: in_progress — implementation half verified (user-requested stage commit). Implemented and green (47 tests across the three herdr modules): `HerdrSubscriptionListener` in `forma_ai/herdr_transport.py` (dedicated long-lived connection per `events.subscribe`, ack validation, persistent read buffer, cross-thread stop), thin adapter methods `snapshot()`/`wait_for_event()` in `forma_ai/herdr_adapter.py`, unit tests over the scripted fake seams, and the live state-transition + reconnect/resubscribe integration test in a uniquely named disposable session. New live-probe fact: `events.wait` matches the pane's CURRENT state, not an event history — a state set with no subscription attached matches immediately, a superseded state never matches, and a never-set state fails closed with error code `timeout`; the no-replay fact from takeover applies only to the subscription path. Environment drift: interactive `/bin/zsh` startup now takes ~62s on this machine, so the P3-T12 live test times out waiting for pane readiness in the full suite (environmental, not a code regression; `git diff` since `63d34af` is purely additive); experimental `SHELL=/bin/sh` server pins were reverted because POSIX sh continues executing a command line after SIGINT, which deterministically breaks the no-leak assertion; recovery candidate is a `/bin/bash` pin pending a SIGINT-per-shell probe
Next action: resume P3-T13 — pin the live-test server shell via the pending SIGINT-per-shell probe (candidate `/bin/bash`: fast startup plus interactive SIGINT aborts the pending line) so the full Python suite is green on this machine, then add the supervisor envelope command for snapshot/event payloads in `scripts/supervisor.py` and the Swift-side `RuntimePresentationProvider` plus `SupervisorProtocol` payload mapping with tests, then full closeout; P4-T14 remains the default next frontend task and unchanged
...
| P3-T12 | verified | Run two real isolated fixture agents through Herdr | integration tests and evidence | Two distinct panes/runs with no input, state, artifact, or cancel leakage | Parallel execution is real |
| P3-T13 | in_progress | Bind snapshot and event subscriptions to the workbench presentation provider | adapter, Swift protocol, UI | Reconnect/resubscribe test and live state transition evidence | UI cannot override Herdr semantic state |
