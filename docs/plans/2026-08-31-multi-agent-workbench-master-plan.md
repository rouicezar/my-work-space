...
Current task: P3-T16 replace Preview agent cards with real Herdr runtime data
Current status: P3-T15 is verified: a discarded client's task is reclaimed by a fresh `HerdrAdapter` only after `session.snapshot()` plus `agent.get` both exactly match the held claim's workspace/pane/terminal/revision/state, stale reclaim attempts fail closed, and the caller can explicitly choose `start_fresh_task()` (same `task_id`, new run/pane/terminal). Native provider-session resume is NOT proven; the previously existing mock-only `resume_task()` (an unproven second `agent.start` mislabeled `native_resume`) was removed. Evidence: `evidence/upstream/herdr-v0.8.2-detach-fresh-run-recovery-verification-2026-09-02.md`. Implementation/evidence commit `e562271` is pushed to `origin/main`.
Next action: begin P3-T16 by defining a failing test that a Swift runtime view renders real Herdr agent data instead of Preview fixtures.
...
- 2026-09-02 21:40 Asia/Shanghai - P3-T15 - Qoder agent - verified `reclaim_task()`/`start_fresh_task()` against a live discarded-client scenario and removed the unverified mock-only `resume_task()` - focused Herdr suite 32 tests, full Python suite, Swift 43 tests with 2 environment-gated Keychain skips, no named-session residue, bilingual parity, `git diff --check` all pass - evidence `evidence/upstream/herdr-v0.8.2-detach-fresh-run-recovery-verification-2026-09-02.md`; commit `e562271` pushed to `origin/main` - next P3-T16
