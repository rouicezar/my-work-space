...
Current task: P3-T12 (verified)
Current status: verified — two real fixture agents ran in parallel through the official Herdr v0.8.2 binary in an isolated named session (`forma-p3t12-test-<uuid8>`, default session untouched). Implemented three thin adapter methods 1:1 over schema methods in `forma_ai/herdr_adapter.py` (`open_workspace`/`workspace.create`, `open_pane`/`pane.split`, `spawn_reported_task` = `pane.send_text` → `pane.report_agent` → `agent.get`), reusing the existing `task_status`/`cancel_task` lifecycle. Live proof: two distinct panes with distinct cwds; parallelism proven by timing (agent B finished inside agent A's 8s sleep window); real Herdr state via `agent.get`/`session.snapshot` (`working` → both `idle`); pane-exact graceful cancel (A's ctrl+c interrupted its sleep; leak steps never executed); no input/artifact/cross-pane-text leakage (executed-output markers verified as `$(date +%s)` timestamp regexes because pane text echoes the typed command). 6 new tests (5 unit + 1 live) red→green; full suite 277 OK, 1 skipped; residue check clean. Evidence: `evidence/upstream/herdr-v0.8.2-two-fixture-agent-isolation-verification-2026-09-01.md`; ledger P3-T12 gap closed and the P3-T10 `pane.split` wording corrected (no command field)
Next action: claim P3-T13 — bind snapshot and event subscriptions (`events.subscribe`/`events.wait`) to the workbench presentation provider, with a reconnect/resubscribe test proving no stale UI after socket loss and live state-transition evidence; P4-T14 remains the default next frontend task and unchanged
