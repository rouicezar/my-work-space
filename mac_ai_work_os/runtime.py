...
class RuntimeManager:
    ...
    def status(self) -> dict[str, object]:
        ...
        if record is None:
            return {"phase": "stopped", "record": None, "omlx_alive": False, "broker_alive": False}
        ...
        if record.phase == "running" and not (omlx_alive and broker_alive):
            effective = "degraded"
        return {
            "phase": effective,
    ...
    def start(
        ...
        *,
        ...
        if existing and existing.phase == "running":
            if existing.omlx and existing.broker and self.controller.matches(existing.omlx) and self.controller.matches(existing.broker):
                return existing
        ...
        if existing and any(
            ...
            for process in (existing.broker, existing.omlx)
        ):
            raise RuntimeManagerError("RUNTIME_RECOVERY_REQUIRED", "managed process remains active")

        created = utc_now()
        record = RuntimeRecord(1, "starting", correlation_id, None, None, None, 1, created, created)
        ...
        try:
            ...
            cleanup_error = None
            seen: set[int] = set()
            for process in (broker_record, omlx_record, omlx_launcher):
    ...
    def stop(self, timeout: float = 10.0) -> RuntimeRecord:
        ...
        if record is None:
            now = utc_now()
            stopped = RuntimeRecord(1, "stopped", "none", None, None, None, 1, now, now)
            self._persist(stopped)
            return stopped
        for process in (record.broker, record.omlx):
            if process and self.controller.matches(process):
                self.controller.terminate(process, timeout)
        stopped = self._next(record, phase="stopped", omlx=None, broker=None, error=None)
        self._persist(stopped)
        return stopped
